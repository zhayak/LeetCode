...
阶乘后零的个数取决于因子5的个数。因子5的个数就是 n//5 + n//25 + n//125 +...

## [204.计数质数.py](204.计数质数.py)
isPrime[i]表示i是否为质数，将isPrime全部初始化为True。从2开始到根号n，如果i是质数的话，那么意味着i，2*i，3*i，...全都为非质数，故将其标记为False。最后数一下isPrime数组中有几个True即可
