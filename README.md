...
## [18.四数之和.py](18.四数之和.py)
先排序，然后拆解成三数之和，再变成两数之和。两数之和采用双指针从两端向中间收缩。注意这里会有重复元素，所以需要内嵌while循环跳过重复元素。
...
## [12.整数转罗马数字.py](12.整数转罗马数字.py)
把特殊的4,9,40,90,400,900也作为一个固定的因子放入mapping中，从大到小将因子从num中减去，转化成对应的罗马数字即可

## [13.罗马数字转整数.py](13.罗马数字转整数.py)
从左至右遍历罗马数字，如果i位置的罗马数字比i+1位置的罗马数字小，则需要减去i位置对应的数字；其他情况则加上i位置对应的数字

## [28.找出字符串中第一个匹配项的下标.py](28.找出字符串中第一个匹配项的下标.py)
这里不使用KMP算法。for循环str1，如果遇到与str2第一个字符匹配的地方，就检测一下后续字符是否也匹配即可
